...
export default function SignIn({ providers, csrfToken  }) {
  ...
  return (
    <>
      ...
      <div className="flex flex-row items-center justify-between w-full h-screen bg-[#131E29]">
        <div className="flex flex-row items-center justify-center w-full h-full">
          <div className="flex flex-col">
            <h1 className="font-black text-5xl text-[#CCB57F]">Kkum</h1>
            <h3 className="font-light text-lg text-[#fff]">
        ...
        <div className="flex flex-col items-center justify-center w-full h-full">
          <div className="flex flex-col items-center justify-center w-full max-w-sm rounded-xl space-y-5">
            <div className="flex flex-col w-full">
              ...
              <h1 className="font-bold text-2xl text-[#fff]">Sign In</h1>
              <form className="flex flex-col w-full space-y-2" method="post" action="/api/auth/signin/email">
                <input name="csrfToken" type="hidden" defaultValue={csrfToken} />
                <input className="px-5 py-3 w-full rounded-lg bg-[#fff] border-2 border-[#1A2D38] focus:outline-none" type="email" id="email" name="email" required />
                <button className="px-5 py-3 w-full rounded-lg shadow-2xl transition ease-in-out duration-300 transform hover:scale-95 text-center text-[#fff] bg-[#1A2D38] focus:outline-none" type="submit">Sign in with Email</button>
              </form>
            </div>
            <div className="flex flex-row items-center w-full font-light text-sm space-x-2">
              <span className="h-0.5 w-full bg-[#fff] opacity-20"></span>
              <span className="font-normal text-sm text-[#fff]">or</span>
              <span className="h-0.5 w-full bg-[#fff] opacity-20"></span>
            </div>
